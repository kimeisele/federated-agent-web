...
**Current state:** spec v0.2 frozen, reference core on `main`, CI green; v0.3 hardening gate complete at `main` commit `0608207`; v0.4 transport-boundary work complete at `main` commit `9bea391`: experimental Nadi/GitHub adapter implemented behind the `Transport` interface, passing the shared conformance suite against a stub relay, plus a controlled cross-account live GitHub rehearsal (evidence in `docs/NADI_LIVE_REHEARSAL.md`). v0.4 gate closes on human review of the rehearsal evidence.
